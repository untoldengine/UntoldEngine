...
namespace U4DEngine{
    ...
    void U4DEPAAlgorithm::determineCollisionManifold(U4DStaticModel* uModel1, U4DStaticModel* uModel2, std::vector<U4DSimplexStruct> uQ){
        ...
        //triangle's normal
        U4DVector3n normal=(face.pointA-face.pointB).cross(face.pointA-face.pointC);
        
        if (normal.dot(origin.toVector())>0) { //if normal in same direction as origin, then negate normal
            normal.negate();
        }
        
        U4DSimplexStruct v=calculateSupportPointInDirection(boundingVolume1, boundingVolume2, normal);

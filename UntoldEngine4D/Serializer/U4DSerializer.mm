...
namespace U4DEngine {
    ...
    std::string U4DSerializer::getFileName(){
        return filename;
    }
    ...
    bool U4DSerializer::serialize(std::string uFileName){
        ...
        if(!uFileName.empty()){
            ...
            logger->log("Scene was saved to file %s",filename.c_str());
            
            return true;
            
        }else{
            
            logger->log("You have not specified a filename. Save the file first.");
...
//    }
    ...
    bool U4DSerializer::deserialize(std::string uFileName){
        ...
        bool loadOk=xmlDoc.LoadFile(uFileName.c_str());
                
        if (!loadOk) {
        
            logger->log("Success: Digital Asset File %s succesfully Loaded.",uFileName.c_str());
            ...
            logger->log("Error: Couldn't load Digital Asset File, No file %s exist.",uFileName.c_str());
...
//        return false;
    ...
    void U4DSerializer::setupScene(){
        ...
        if(sceneConfig->getScenePropsBehavior("camera", "Scene", "position", &position[0], 3)){
            camera->translateTo(position[0], position[1], position[2]);
        }
        
        if(sceneConfig->getScenePropsBehavior("camera", "Scene", "orientation", &orientation[0], 3)){
            camera->rotateTo(orientation[0], orientation[1], orientation[2]);
        }
        
        U4DDirectionalLight *light=U4DDirectionalLight::sharedInstance();
        
        
        if(sceneConfig->getScenePropsBehavior("light", "Scene", "position", &position[0], 3)){
            light->translateTo(position[0], position[1], position[2]);
        }
        
        if(sceneConfig->getScenePropsBehavior("light", "Scene", "orientation", &orientation[0], 3)){
            light->rotateTo(orientation[0], orientation[1], orientation[2]);
        }
        
        if(sceneConfig->getScenePropsBehavior("light", "Scene", "color", &color[0], 3)){
            light->rotateTo(color[0], color[1], color[2]);
        }
        
        //get number of entities
        int count=sceneConfig->getEntitiesCount();
        
        if(count>0){
            
            tinyxml2::XMLElement *entities=sceneConfig->root->FirstChildElement("Entities");
            
            for (tinyxml2::XMLElement *child=entities->FirstChildElement("entity"); child!=NULL; child=child->NextSiblingElement("entity")) {
            
                //get name and ref name
                std::string entityName=child->Attribute("name");
                std::string entityRefName=child->Attribute("refName");
                
                sceneConfig->getEntityBehavior(entityName.c_str(), "Space", "position", &position[0], 3);
                sceneConfig->getEntityBehavior(entityName.c_str(), "Space", "orientation", &orientation[0], 3);
                
                U4DVector3n pos(position[0],position[1],position[2]);
                U4DVector3n orient(orientation[0],orientation[1],orientation[2]);
    
                //Ask the factory class to create an instance of each object and load it into the world
    
                entityFactory->createModelInstance(entityRefName,entityName, "U4DModel","modelpipeline", pos, orient);

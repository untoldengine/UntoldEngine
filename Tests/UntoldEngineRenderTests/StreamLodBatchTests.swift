...
/// These test the CPU registry behaviour for LOD group entities without GPU or disk I/O.
@MainActor
final class StreamLodBatchOOCIntegrationTests: XCTestCase {
    ...
    private func makeEntry(name: String) -> ProgressiveAssetLoader.CPUMeshEntry {
        guard let device = MTLCreateSystemDefaultDevice() else {
            fatalError("No Metal device")
        }
        return ProgressiveAssetLoader.CPUMeshEntry(
            object: MDLObject(),
    ...
    func testHasCPULODData_detectsLODOOCEntity() {
        let groupEntityId: EntityID = 5000
        loader.storeCPULODMesh(makeEntry(name: "Tree_LOD0"), for: groupEntityId, lodIndex: 0)
        loader.storeCPULODMesh(makeEntry(name: "Tree_LOD1"), for: groupEntityId, lodIndex: 1)

        XCTAssertTrue(loader.hasCPULODData(for: groupEntityId),
                      "hasCPULODData must return true for a LOD+OOC entity")
    }

    func testHasCPULODData_distinguishesFromRegularOOCEntity() {
        let lodEntityId: EntityID = 5010
        let regularEntityId: EntityID = 5011

        loader.storeCPULODMesh(makeEntry(name: "Tree_LOD0"), for: lodEntityId, lodIndex: 0)
        loader.storeCPUMesh(makeEntry(name: "Building#0"), for: regularEntityId)

        XCTAssertTrue(loader.hasCPULODData(for: lodEntityId),
                      "LOD+OOC entity must report hasCPULODData = true")
        XCTAssertFalse(loader.hasCPULODData(for: regularEntityId),
                       "Regular OOC entity must report hasCPULODData = false")
    }

    func testLODOOCRegistration_multiGroupChildrenAreMappedToRoot() {
        let rootId: EntityID = 5020
        let treeGroupId: EntityID = 5021
        let rockGroupId: EntityID = 5022

        // Register LOD entries for two groups
        loader.storeCPULODMesh(makeEntry(name: "Tree_LOD0"), for: treeGroupId, lodIndex: 0)
        loader.storeCPULODMesh(makeEntry(name: "Tree_LOD1"), for: treeGroupId, lodIndex: 1)
        loader.storeCPULODMesh(makeEntry(name: "Rock_LOD0"), for: rockGroupId, lodIndex: 0)
        loader.storeCPULODMesh(makeEntry(name: "Rock_LOD1"), for: rockGroupId, lodIndex: 1)
        loader.registerChildren([treeGroupId, rockGroupId], for: rootId)

        // Both groups should have LOD data
        XCTAssertTrue(loader.hasCPULODData(for: treeGroupId))
        XCTAssertTrue(loader.hasCPULODData(for: rockGroupId))

        // Children lookup should return both group entities
        let children = loader.getChildren(for: rootId)
        XCTAssertEqual(Set(children), Set([treeGroupId, rockGroupId]))
    }

    func testLODOOCRegistration_allLevelsRestoredAfterCancelAll() {
        let entityId: EntityID = 5030
        loader.storeCPULODMesh(makeEntry(name: "Prop_LOD0"), for: entityId, lodIndex: 0)
        loader.storeCPULODMesh(makeEntry(name: "Prop_LOD1"), for: entityId, lodIndex: 1)
        loader.storeCPULODMesh(makeEntry(name: "Prop_LOD2"), for: entityId, lodIndex: 2)

        loader.cancelAll()

        XCTAssertNil(loader.retrieveAllCPULODMeshes(for: entityId),
                     "cancelAll must clear all LOD registry entries")
        XCTAssertFalse(loader.hasCPULODData(for: entityId))
    }

    func testLODStubLevelStartsNotResident() {
        // Verify that a LODLevel created with empty mesh starts .notResident —
        // the automatic residency state that the LOD+OOC path relies on.
        let stubLevel = LODLevel(mesh: [], maxDistance: 50, url: URL(fileURLWithPath: "/dev/null"), assetName: "Tree_LOD0")
        XCTAssertEqual(stubLevel.residencyState, .notResident,
                       "Stub LODLevel (empty mesh) must start as .notResident")
    }
}

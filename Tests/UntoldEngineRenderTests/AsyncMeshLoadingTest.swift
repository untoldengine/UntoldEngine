...
final class AsyncMeshLoadingTest: BaseRenderSetup {
    ...
    // MARK: - loadScene Tests

    func testLoadScene_completionCalledWithTrueOnSuccess() async {
        // Given: A valid USDZ file
        let expectation = XCTestExpectation(description: "loadScene completed")
        var loadSuccess = false

        // When: Load a scene
        loadScene(filename: "ball", withExtension: "usdz") { success in
            loadSuccess = success
            expectation.fulfill()
        }

        // Then: Completion should be called with true
        await fulfillment(of: [expectation], timeout: 10.0)
        XCTAssertTrue(loadSuccess, "loadScene should succeed for a valid USDZ file")
    }

    func testLoadScene_rootEntityNamedAfterFilename() async {
        // Given: A valid USDZ file
        let expectation = XCTestExpectation(description: "loadScene completed")

        // When: Load a scene
        loadScene(filename: "ball", withExtension: "usdz") { _ in
            expectation.fulfill()
        }

        // Then: The root entity should be named after the filename
        await fulfillment(of: [expectation], timeout: 10.0)
        XCTAssertNotNil(findEntity(name: "ball"), "Root entity should be named after the filename")
    }

    func testLoadScene_createsDefaultCameraWhenImportCamerasDisabled() async {
        // Given: importCameras: false (default)
        let expectation = XCTestExpectation(description: "loadScene completed")

        // When: Load without importing cameras
        loadScene(filename: "ball", withExtension: "usdz", importCameras: false) { _ in
            expectation.fulfill()
        }

        // Then: A default camera should be created and set as active
        await fulfillment(of: [expectation], timeout: 10.0)
        XCTAssertNotNil(CameraSystem.shared.activeCamera, "Active camera should be set after loadScene")
        XCTAssertNotNil(findEntity(name: "Game Camera"), "Default camera entity should be named 'Game Camera'")
    }

    func testLoadScene_createsDefaultLightWhenImportLightsDisabled() async {
        // Given: importLights: false (default)
        let expectation = XCTestExpectation(description: "loadScene completed")

        // When: Load without importing lights
        loadScene(filename: "ball", withExtension: "usdz", importLights: false) { _ in
            expectation.fulfill()
        }

        // Then: A default directional light entity should be created
        await fulfillment(of: [expectation], timeout: 10.0)
        XCTAssertNotNil(findEntity(name: "Directional Light"), "Default light entity should be named 'Directional Light'")
    }

    func testLoadScene_destroysPreviousSceneOnReload() async {
        // Given: A scene loaded with one asset
        let firstExpectation = XCTestExpectation(description: "First load completed")
        loadScene(filename: "ball", withExtension: "usdz") { _ in firstExpectation.fulfill() }
        await fulfillment(of: [firstExpectation], timeout: 10.0)
        XCTAssertNotNil(findEntity(name: "ball"), "First scene root entity should exist before reload")

        // When: A different scene is loaded
        let secondExpectation = XCTestExpectation(description: "Second load completed")
        loadScene(filename: "grass", withExtension: "usdz") { _ in secondExpectation.fulfill() }
        await fulfillment(of: [secondExpectation], timeout: 30.0)

        // Then: The previous scene's root entity should be gone; the new one should exist
        XCTAssertNil(findEntity(name: "ball"), "Previous scene root entity should be destroyed after reload")
        XCTAssertNotNil(findEntity(name: "grass"), "New scene root entity should exist after reload")
    }

    func testLoadScene_activeCameraRemainsSetAfterReload() async {
        // Given: An initial scene is fully loaded
        let firstExpectation = XCTestExpectation(description: "First load completed")
        loadScene(filename: "ball", withExtension: "usdz") { _ in firstExpectation.fulfill() }
        await fulfillment(of: [firstExpectation], timeout: 10.0)
        ...
        let secondExpectation = XCTestExpectation(description: "Second load completed")
        loadScene(filename: "ball", withExtension: "usdz") { _ in secondExpectation.fulfill() }
        await fulfillment(of: [secondExpectation], timeout: 10.0)

        // Then: Active camera should be valid after the reload completes
        XCTAssertNotNil(CameraSystem.shared.activeCamera, "Active camera must be set after reload")
    }

    func testLoadScene_invalidFilenameCallsCompletionWithFalse() async {
        // Given: A filename that doesn't resolve to a real file
        let expectation = XCTestExpectation(description: "loadScene completed with failure")
        var loadSuccess = true

        // When: Load a non-existent scene
        loadScene(filename: "nonexistent_scene_xyz", withExtension: "usdz") { success in
            loadSuccess = success
            expectation.fulfill()
        }

        // Then: Completion should be called with false
        await fulfillment(of: [expectation], timeout: 10.0)
        XCTAssertFalse(loadSuccess, "loadScene should report failure for a non-existent file")
    }

    func testLoadScene_invalidFilenameStillProvidesDefaultCameraAndLight() async {
        // Given: A filename that doesn't resolve to a real file
        let expectation = XCTestExpectation(description: "loadScene completed with failure")

        // When: Load a non-existent scene
        loadScene(filename: "nonexistent_scene_xyz", withExtension: "usdz") { _ in
            expectation.fulfill()
        }

        // Then: Default camera and light should still be created despite the load failure
        await fulfillment(of: [expectation], timeout: 10.0)
        XCTAssertNotNil(CameraSystem.shared.activeCamera, "Default camera should be created even when load fails")
        XCTAssertNotNil(findEntity(name: "Directional Light"), "Default light should be created even when load fails")
    }

    func testLoadScene_enableBatching_completesSuccessfully() async {
        // Given: enableBatching: true
        let expectation = XCTestExpectation(description: "loadScene with batching completed")
        var loadSuccess = false

        // When: Load a scene with batching enabled
        loadScene(filename: "ball", withExtension: "usdz", enableBatching: true) { success in
            loadSuccess = success
            expectation.fulfill()
        }

        // Then: Load should succeed and the root entity should exist
        await fulfillment(of: [expectation], timeout: 10.0)
        XCTAssertTrue(loadSuccess, "loadScene with enableBatching should succeed")
        XCTAssertNotNil(findEntity(name: "ball"), "Root entity should exist after a batched load")
    }

    func testLoadScene_enableGeometryStreaming_completesSuccessfully() async {
        // Given: enableGeometryStreaming: true (forces .outOfCore policy)
        let expectation = XCTestExpectation(description: "loadScene with streaming completed")
        var loadSuccess = false

        // When: Load a scene with geometry streaming enabled
        loadScene(filename: "ball", withExtension: "usdz", enableGeometryStreaming: true) { success in
            loadSuccess = success
            expectation.fulfill()
        }
        ...
        await fulfillment(of: [expectation], timeout: 10.0)
        XCTAssertTrue(loadSuccess, "loadScene with enableGeometryStreaming should succeed")
        XCTAssertNotNil(findEntity(name: "ball"), "Root entity should exist after a streaming load")
    }
}

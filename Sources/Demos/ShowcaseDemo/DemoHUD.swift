        ...
        private var controlsPanel: some View {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Picker("G-Buffer View", selection: $state.renderDebugView) {
                    ...
                    Text("Roughness").tag(RenderDebugViewMode.roughness)
                    Text("Metallic").tag(RenderDebugViewMode.metallic)
                    Text("Depth").tag(RenderDebugViewMode.depth)
                    Text("SSAO (Blurred)").tag(RenderDebugViewMode.ssaoBlurred)
                    Text("Pre-tonemap HDR Luma").tag(RenderDebugViewMode.preTonemapHDRLuminance)
                    Text("Post-tonemap Output").tag(RenderDebugViewMode.postTonemapOutput)

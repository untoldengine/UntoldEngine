...
- [Patch] Deterministic start order for concurrent Gaussian page reads (0efe74a…)
- [Patch] Project splats with the reference rasterizer's 0.3 px dilation (b1e39c7…)
- [Patch] Cap the splats blended per pixel per platform: 128 on a Mac (8ffb5f9…)
- [Patch] Keep splat pixels out of FXAA and SMAA (90973b0…)
- [Patch] Rebuild the Metal kernels for the dilation and the anti-aliasing mask (15b2ab5…)
- [Patch] Review follow-ups: the mask follows the Gaussian pass, and the tests say more (e5294b8…)
- [Patch] Blend the splat layer in the capture's own colour space (6dc9aba…)
- [Patch] Coarsen colours in the blend space (835e198…)
- [Patch] Keep splat pixels out of the look pass's grade and tone map (4ab8f7f…)
- [Patch] Draw a tilted splat's quad on its ellipse, not its mirror image (fc3539f…)
- [Patch] Apply the colour gain in linear light (7cc1e67…)
- [Patch] Look pass: grade only the scene behind a partly covered splat pixel (e4c1568…)
- [Patch] Coarsener: drop the unused linear-colour parameter, fix stale notes (9414784…)
- [Patch] Raise the splat screen-radius ceiling to the viewport, as a runtime limit (f881b87…)
- [Patch] Cook: turn the spherical harmonics with the splats (55ba679…)
- [Patch] A crisp splat kernel switch for A/Bs against other viewers (a8f52c7…)
- [Patch] Review follow-ups: a zero radius ceiling means the viewport, basis pinned (74be454…)
- [Patch] Harmonic rotation test: the CPU evaluator clamps a negative colour at zero (a45aeed…)
- [Patch] Review follow-ups: one alpha floor for both un-premultiply divides, named coverage texture indices (3e8fbe8…)
- [Patch] Blend test: compare the untinted red channel with the shared tolerance (5a32ba4…)
### 📚 Docs

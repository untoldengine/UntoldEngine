...
In a stereo frame the chunk test uses the previous frame's two eye view-projections (`renderInfo.xrEye0ViewProjection` / `xrEye1ViewProjection`, written per eye by `renderXR`), and a chunk is visible when **either** eye keeps it — never the single head-centre view the per-splat passes run with, which would drop a chunk only the other eye sees. `GaussianDebugOptions.shared.disableChunkCull` appends every chunk (the per-splat pass then walks the whole asset through the same indirect path) for A/B runs. Entities without a chunk table — `.ply` files, `.untoldgs` decoded on the CPU — keep the whole-buffer `gaussianFrustumCull`.

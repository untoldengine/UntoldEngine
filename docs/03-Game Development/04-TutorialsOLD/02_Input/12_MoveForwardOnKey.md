...
4) **Build & test**
- In Xcode, run the GenerateScripts target to build the scripts, attach `MoveForwardOnKey.uscript` to an entity, press **Play**, then hold **W** to move forward. (USC scripts are authored in Xcode; the editor does not include a built-in script editor.)

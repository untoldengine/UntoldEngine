...
Phase 2 (tiers as chunk ranges in one file, merged coarse levels, object budgets, the window bake) and phase 3 (streamed environment) follow as their own series once PRs 1–7 are merged upstream. The next items — disk paging within an asset (range loads, a page pool, residency) and per-chunk LOD tiers (tiers as chunk ranges, merged coarse levels) — build on the per-chunk screen area row 9 introduces: it is the weight a pager and a tier pick need.

...
**Status:** Rows 2–8 of §8 are implemented (fork and upstream); the chunk-level cull that row 6 planned shipped separately as Stage 1 of `feature/gaussian_budgeted_working_set` (row 8). Rows 9–11 — the screen-weighted quotas, the disk paging and the per-chunk LOD tiers — are implemented on the fork branch `feature/gaussian_chunk_paging` and travel as one PR. Of phase 2 the merged coarse levels shipped as row 11 (per chunk, inside the v3 file); the tiers as chunk ranges of one file, the root level, the object budgets and the window bake, and phase 3, remain proposals.
...
| Registration | `setEntityGaussian(entityId:filename:withExtension:)` (sync), `setEntityGaussianAsync`, `setEntityGaussian(source: .single / .progressive)`, `setEntityGaussianTileStreaming(source:options:)` which attaches the entity to its containing tile. PLY and `.untoldgs` both accepted. Load-time culling of negligible-opacity splats. | `RegistrationSystem.swift:3745–4010` |
...
- Progressive tiers are separate files, so an asset with four tiers stores its coarse splats up to four times and a tier switch is a whole-file load. (Row 11 puts one or two merged coarse levels of every chunk inside the same file, in an optional section an older reader skips, so a far or not-yet-loaded chunk draws a few merged splats without a second file; the whole-file tiers stay separate files.)
...
3. **Tiers.** `--lod-levels N` keeps producing nested tiers, now as v3 files; phase 2 folds the tiers into one file as `lodLevel` chunk ranges under the same tree, and adds merged coarse levels (opacity × area weights, moment-matched covariance) with a root level ≤ 32 K splats (the merged coarse levels are in place since row 11, per chunk and inside the same file — `--splat-coarse-levels`, `--splat-coarse-ratio-log2`, [untoldgsFormat.md](../Architecture/untoldgsFormat.md) — with the weights and the moment matching as written and the opacity remap applied at cook time; the chunk-range tiers and the root level remain). The budget table (splat count per level, screen-height switch points) is written into the `gaussianAsset` record.
...
| Detail jumping when walking toward an object | LOD popping | Level by projected size, not distance; parent–child opacity remap `1 − (1 − α_parent)^(1/K)` interpolated over a band of τ; hysteresis per node; camera-position cross-fade between independent sets. (In place since row 11 for the per-chunk levels: the level follows the chunk's screen area through the frame's density cap in half-octave tiers — fine, level 1 from two octaves under the chunk's own density, level 2 from five — with a one-tier hysteresis band, and every switch cross-fades over 16 frames at the coverage-preserving weights `1 − (1 − α)^w` in and `1 − (1 − α)^(1 − w)` out, so the transmittance of a surface both levels cover never dips; the remap is baked into the merged records as the coverage opacity `1 − exp(−Σ αᵢ sᵢ / s_M)`. The whole-file `_lodN` tiers keep the distance thresholds of `GaussianLODSystem`.) |
| A region snapping from blurry to sharp | Streaming popping | Coarse level always resident (in place since row 11: the coarse section streams through the pager's queue coarsest level first, outside the page pool, and a chunk with nothing resident is listed for its finest landed level and draws it — level 1 once its piece has landed, level 2 before — from the frame after it is first listed, faded in, the fine head fading in over it when it lands); arriving chunks fade in over 150–300 ms (in place since row 10: an arriving tier fades in over 16 frame-counted frames, and a paged tier warms before the LOD switch); prefetch by velocity look-ahead (open); hold the current level under a stall. |
| Shimmer on fine detail | Aliasing: sub-pixel splats; the shader's low-pass is the reference rasterizer's 0.3 px since 2026-09 (0.1 px before) | Mip-Splatting: 3D smoothing filter at cook time sized to capture density, 0.3 px dilation at render time, `antialiased` flag in the header; coarser LOD removes sub-pixel splats at distance. |
...
- Every registration entry point keeps its signature and behaviour: `setEntityGaussian(entityId:filename:withExtension:)`, `setEntityGaussianAsync`, `setEntityGaussian(source:)`, `setEntityGaussianTileStreaming(source:options:)`. `.ply` keeps loading directly.
...
| 11 | `feature/gaussian_chunk_paging` (the branch of rows 9 and 10; one PR) | [Feature] Per-chunk LOD tiers: merged coarse levels in an optional `.untoldgs` v3 section | The cooker bakes one or two importance-sorted merged levels of every chunk (`UntoldGSCoarsener`: a weighted Lloyd clustering over the chunk's Morton order with a linear-space colour term, one moment-matched Gaussian per cluster, the coverage opacity `1 − exp(−Σ αᵢ sᵢ / s_M)`, level 2 built over level 1; ratios `3, 6` give n/8 and n/64; `--splat-coarse-levels` (`auto`, `0`, `1`, `2`) and `--splat-coarse-ratio-log2`, automatic for tiers of at least 64 chunks) into a page-aligned section after the last chunk payload, advertised by `hasCoarseLevels` and header words carved from the reserved tail and indexed by 64-byte `UntoldGSChunkEntry` rows with their own decode ranges and CRCs, so a reader that predates it draws the fine records unchanged. At runtime the coarse records live per entity outside the page pool (`GaussianCoarseTable`: both levels when they fit 20 % of the residency budget, else the coarsest alone, else none), streamed coarsest level first through the pager's read queue for a paged entity; `gaussianChunkCull` lists a non-resident chunk for its finest landed level and bins every levelled chunk by its full density with the counts each regime would draw; `gaussianComputeBudgetScale` folds the level rule into its bounded request `R(d)`; `gaussianComputeChunkQuotas` chooses each chunk's level from the solved cap by integer tier arithmetic (`tier(min(cap, floor)) − tier(n / A)` against the tier shifts `2 × (ratio − 1)` from the file, one tier of hysteresis, a level not available stepping coarser; the floor `maxSplatsPerPixel` (1) × viewport pixels draws the far field coarse on a fitting frame too) and runs a two-phase switch (detect, reserve, commit) whose outgoing window rides as a second tagged visible-chunk entry (bits 24–26 of `chunkIndex`) reserved in `transitionSplats`; `gaussianChunkDecodePreprocess` draws a coarse level from the coarse records against the level's own constants and cross-fades the two windows over 16 frames at `1 − (1 − α)^w` / `1 − (1 − α)^(1 − w)`; the pager wants no fine rank for a chunk drawn coarse (its tiers leave as surplus) and shields a fading chunk's tiers from eviction. `gaussianLevelMode` (`.auto`, `.fineOnly`, `.coarseOnly`), `disableLevelCrossFade`, `levelDebugTint`, `maxSplatsPerPixelOverride`, `coarseBudgetFractionOverride`. | 10 |

Phase 2 (tiers as chunk ranges in one file, object budgets, the window bake) and phase 3 (streamed environment) follow as their own series once PRs 1–7 are merged upstream. Row 10 shipped the disk paging within an asset (range loads, a page pool, residency) and row 11 the merged coarse levels per chunk, both on the per-chunk screen area row 9 introduced: it is the weight a pager and a level pick need. The follow-ups of row 11, in order: pooled coarse levels (the coarsest level pinned inside the pool, level 1 demand-paged through the pager on its 64-byte entries — lifts the level-2-only outcome of a 20 M asset on a 300 MiB geometry budget and charges the coarse bytes to the residency budget instead of beside it); merged levels per node of the preorder tree with a pinned root read at load (the root ≤ 32 K above: a representation below level 2's floor and a first image for a paged entity before any piece lands); spherical harmonics on coarse records (`coarseFlags` bit 0) if the near-fade colour residual proves visible; `untoldengine gaussian-coarsen` for files cooked before the section; the editor's "Coarse levels" cook-sheet row and level stats in the Splat inspector; a cull box widened by the level's own `logScaleMax` if the edge residual proves visible; a velocity-predicted level choice and a foveated floor, with the paging follow-ups.

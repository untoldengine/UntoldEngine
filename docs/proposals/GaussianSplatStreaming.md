...
| 8 | `feature/gaussian_budgeted_working_set` | [Feature] Budgeted working set fed by chunk-level culling | Stage 1 (shipped): the `.untoldgs` chunk table is retained on the component; `gaussianChunkCull` tests each chunk's centre AABB padded by `kGaussianQuadSigma·exp(logScaleMax)` against the guard-banded frustum (either eye in stereo, both rebuilt with the current scene root) and the previous frame's HZB; the per-splat cull runs indirectly one threadgroup per visible chunk into the unchanged visible-index list, so the image is identical to the whole-buffer path; `GaussianDebugOptions.disableChunkCull`. Stage 2 (pending): 16-byte resident records with a fused decode/project pass, the budget-sized shared set with per-chunk quotas, opacity band and hysteresis, accounting. | 6 |

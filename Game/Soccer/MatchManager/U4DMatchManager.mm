...
namespace U4DEngine {
    ...
    U4DMatchManager::U4DMatchManager():ballOutOfBound(false),state(-1),teamAScore(0),teamBScore(5),elapsedClockTime(0){
        
        teamA=new U4DTeam("TeamA");
        teamB=new U4DTeam("TeamB");
        
        //Create the callback. Notice that you need to provide the name of the class
        endGameScheduler=new U4DCallback<U4DMatchManager>;

        //create the timer
        endGameTimer=new U4DTimer(endGameScheduler);
        
        //Create the callback. Notice that you need to provide the name of the class
        startGameScheduler=new U4DCallback<U4DMatchManager>;

        //create the timer
        startGameTimer=new U4DTimer(startGameScheduler);
...
//        }
    ...
    void U4DMatchManager::initTextElements(U4DText *uScoreA, U4DText *uScoreB, U4DText *uGameClock, U4DText *uBroadCastMessage){
        
        scoreA=uScoreA;
        scoreB=uScoreB;
        gameClock=uGameClock;
        broadcastMessage=uBroadCastMessage;
        std::string clockTimeString="89:";
        
        
        clockTimeString+="0"+std::to_string(elapsedClockTime);
        
        
        scoreA->setText((std::to_string(teamAScore)).c_str());
        scoreB->setText((std::to_string(teamBScore)).c_str());
        gameClock->setText((clockTimeString).c_str());
        broadcastMessage->setText("playing");
    ...
    void U4DMatchManager::timesUp(){
        ...
        if(elapsedClockTime<10){
            clockTimeString+="0"+std::to_string(elapsedClockTime);
        }else{
            clockTimeString+=std::to_string(elapsedClockTime);
        }
...
//        scoreAndOutOfBoundScheduler->scheduleClassWithMethodAndDelay(this, &U4DMatchManager::checkIfScoreAndOutOfBound, scoreAndOutOfBoundTimer, 0.5, true);
    ...
    void U4DMatchManager::changeState(int uState){
        ...
        switch(uState){
            ...
            case outOfBound: 
                broadcastMessage->setText("Out of bound");
                break;
            ...
            case goalScored:
                broadcastMessage->setText("Gooooool");
                break;
            ...
            case gameTimeReached:
                broadcastMessage->setText("Game Over!");
                break;

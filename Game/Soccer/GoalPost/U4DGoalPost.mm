...
namespace U4DEngine{
    ...
    bool U4DGoalPost::isBallInsideGoalBox(){
        ...
        if(!goalBoxComputed){
            ...
            goalBoxDimensions=m*goalBoxDimensions;
            U4DPoint3n center=getAbsolutePosition().toPoint();
            
            U4DAABB tempGoalBoxAABB(std::abs(goalBoxDimensions.x),std::abs(goalBoxDimensions.y),std::abs(goalBoxDimensions.z),center);
